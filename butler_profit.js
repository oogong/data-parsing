...
async function fetchData() {
  const url = `https://api.butler.works/api/analysis/fundamentals/profitability/summary?corpCode=00126380&fsDiv=MFS&quarterPeriod=accumulated`;
...
async function main() {
  ...
  const filteredData = data
    ...
    .map((item) => ({
      date: item.date,
      proROE: item.proROE, 
      proOperatingProfitMargin: item.proOperatingProfitMargin,
      proNetProfitMargin: item.proNetProfitMargin,
    }));

  fs.writeFileSync("stock_profit.json", JSON.stringify(filteredData, null, 2));
}
